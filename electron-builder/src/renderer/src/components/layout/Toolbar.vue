...
async function selectBackground() {
  try {
    const result = await window.electron.ipcRenderer.invoke('dialog:open-file', {
      title: t('dialogs.selectBackgroundImage'),
      filters: [{ name: t('dialogs.imageFiles'), extensions: ['png', 'jpg', 'jpeg', 'bmp'] }]
    })
    if (result) {
...
function bringToFront() {
  if (projectStore.selectedElementId) {
    ...
    uiStore.setStatus(t('status.bringToFront'))
  }
}
...
function sendToBack() {
  if (projectStore.selectedElementId) {
    ...
    uiStore.setStatus(t('status.sendToBack'))
  }
}
</script>
...
<template>
  <div class="toolbar">
    ...
    <div class="toolbar-group">
      ...
      <button class="tool-btn icon-only" @click="uiStore.zoomIn" :title="$t('tooltips.zoomIn')">
      ...
      <button class="tool-btn icon-only" @click="uiStore.resetZoom" :title="$t('tooltips.zoomReset')">
